...
with open("zuil.csv", 'w', newline='') as f:
...
def output_csv():
    with open("zuil.csv", "a", newline='') as f:
        csv_writer = csv.writer(f)
        csv_writer.writerow([naam, bericht, time.strftime('%H:%M:%S, %d %b %y'), stations[random.randint(0, 2)]])
